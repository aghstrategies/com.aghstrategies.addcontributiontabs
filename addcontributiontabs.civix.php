 ...
 */
function _addcontributiontabs_civix_civicrm_xmlMenu(&$files) {
  foreach (_addcontributiontabs_civix_glob(__DIR__ . '/xml/Menu/*.xml') as $file) {
    $files[] = $file;
  }
}
 ...
 * @return array(string)
 */
function _addcontributiontabs_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_addcontributiontabs_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
 ...
 */
function _addcontributiontabs_civix_civicrm_managed(&$entities) {
  $mgdFiles = _addcontributiontabs_civix_find_files(__DIR__, '*.mgd.php');
  foreach ($mgdFiles as $file) {
    $es = include $file;
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'com.aghstrategies.addcontributiontabs';
      }
      $entities[] = $e;
    }
  }
 ...
 */
function _addcontributiontabs_civix_glob($pattern) {
  $result = glob($pattern);
 ...
 */
function _addcontributiontabs_civix_insert_navigation_menu(&$menu, $path, $item, $parentId = NULL) {
  static $navId;
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _addcontributiontabs_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
