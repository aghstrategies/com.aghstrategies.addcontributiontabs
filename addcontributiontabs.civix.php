 ...
 * extension.
 */
class CRM_Addcontributiontabs_ExtensionUtil {
  const SHORT_NAME = 'addcontributiontabs';
  const LONG_NAME = 'com.aghstrategies.addcontributiontabs';
  const CLASS_PREFIX = 'CRM_Addcontributiontabs';
 ...
 * @return bool
 */
function _addcontributiontabs_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _addcontributiontabs_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _addcontributiontabs_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _addcontributiontabs_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _addcontributiontabs_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _addcontributiontabs_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _addcontributiontabs_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
